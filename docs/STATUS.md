...
- stable run `30636412439`（commit `0acd0d8`）成功，artifact `8800636377`
...
- 两轮网络 HIL 均能枚举 RNDIS，但 Windows 报告 media disconnected；只读
  overlay 分析确认 F2FS ready、S25 服务和 LAN/DHCP 配置正确；pre-bind 诊断
  证明内核只在首次 UDC 绑定后创建 `usb0`，新候选改用首次绑定创建接口、准备
  LAN 后重绑 UDC，等待设备复测。
